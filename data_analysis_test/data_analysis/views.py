...
def save_data(request):
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        sql = 'insert into `basic_data`(`城市`,`项目立项年份`,`土地面积(亩)`, `工业用地费用(万/亩)`, `商业用地(万/亩)`, `住宅用地(万/亩)`, `机架数量（个）`, `单机架IT功率（KW）`, `建筑物造价（元/m2)`, `单KW机电设备建设成本（万元）`, `单KW机电工程建设成本（万元）`, `终局上架率`, `机架上满月数`, `单KW价格(含税，RMB/月/KW)`, `PUE`, `同时系数`, `电费（元/度, 含税)`, `水费（元/立方米）`, `耗水率`, `宽带租金（元/1G/月)`, `单机架配带宽 （100M/机架，共享）`, `新增人员数量（管理+运维）`, `年人均工资（万元）`, `工资年增长率`, `职工福利费（占工资比例）`, `低值易耗品（办公用品，占职工福利费）`, `经营管理费（占营业收入）`, `固定资产折旧年限（年）`, `土地建筑 （年）`, `设备（年）`, `固定资产残值率`, `固定资产折旧方法`, `增值税税率`, `机电设备`, `安装工程`, `电，油增值税`, `水增值税`, `企业所得税率`, `借贷利息`, `贷款年限`, `贷款配比`, `贷款手续费`, `折现率`, `增值税`, `机柜租赁增值税（不动产）`, `带宽服务增值税`, `云计算服务增值税`, `增值税附加税率`, `土地增值税`, `建筑工程税费`) ' \
...
def post_data(request):
    dir = {}
    if request.POST:
        dir['city'] = request.POST['city']
    return render(request, 'post.html', dir)
